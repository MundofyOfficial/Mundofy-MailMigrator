...
Mundofy MailMigrator was created with an uncompromising commitment to privacy and data security. This guide details how the application handles your data, its standalone architecture, and its compliance with global data protection laws.

---

## 1. Zero-Telemetry & Zero-Tracking Architecture

Unlike many modern cloud-based migration services that require you to upload your server credentials to an external dashboard, **Mundofy MailMigrator runs 100% locally on your computer**:

* **Direct Point-to-Point TLS Handshakes:** All network connections are opened directly between your local PC, your source email server, and your destination email server.
* **In-Memory Streaming:** Messages and folder structures are streamed directly in-memory from the source to destination. No email files, attachments, or passwords are saved to temporary cache files on your hard drive.
...
* **Zero Analytics & Diagnostic Siphoning:** The application contains no Google Analytics, no telemetry trackers, no user behavior monitoring, and no automated crash dump uploads.
...
## 2. Worldwide Privacy Law Compliance
...
## 3. Complete Network Transparency: The One Single Ping
...
* **Purpose:** Queries the public GitHub Releases API to compare the latest published tag against your installed version string (e.g. 1.4.0), notifying you if an update is available with security fixes or improvements.
* **Data Transmitted:** An anonymous HTTP request with a standard User-Agent header (Mundofy-MailMigrator/1.4.0). **No IP logging, no machine GUIDs, no usernames, and zero telemetry data.**
...
## 4. Open-Source Independent Verifiability
...
2. **Network Packet Capture:** You can monitor network traffic in real-time with tools like Wireshark or Microsoft Message Analyzer to verify that only encrypted sockets to your mail servers and the anonymous GitHub version check occur.
